...
生成后，`docs/api/openapi.json` 应新增以下 7 个 API 路径：
...
- `GroupActionRequest` — 归组操作通用请求
- `GroupMoveMemberRequest` — 成员移动请求
- `GroupMoveMemberResponse` — 成员移动响应
...
required_paths = [
    ...
    '/api/groups/{group_id}/move-member',
]
...
git commit -m "Phase 3.8: 更新 openapi.json — 新增归组 API 7 个路径 + 12 个 Schema"
```
