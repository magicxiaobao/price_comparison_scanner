...
| 编号 | 文件名 | 画面内容 | 目标 URL | 前置状态要求 | 用途章节 | 来源优先级 | 设计图 fallback |
|------|--------|----------|----------|--------------|----------|------------|-----------------|
...
| 03 | `03-import-stage.png` | 文件导入阶段 | `http://localhost:5173/#/project/:project-id` | 工作台已加载，导航至阶段 0（导入阶段），已显示已上传文件列表 | §5.1 文件导入 | live-capture | `docs/design/ui/import-stage/screen.png` |
| 04 | `04-standardize-stage.png` | 标准化阶段表格 | `http://localhost:5173/#/project/:project-id` | 工作台已加载，导航至阶段 1（标准化阶段），标准化表格已显示数据行 | §5.2 字段标准化 | live-capture | `docs/design/ui/standardize-stage/screen.png` |
| 05 | `05-grouping-stage.png` | 商品归组界面 | `http://localhost:5173/#/project/:project-id` | 工作台已加载，导航至阶段 2（归组阶段），商品分组卡片已显示 | §5.3 商品归组 | live-capture | `docs/design/ui/grouping-stage/screen.png` |
| 06 | `06-compliance-stage.png` | 符合性审查界面（矩阵已加载） | `http://localhost:5173/#/project/:project-id` | 工作台已加载，导航至阶段 3（符合性审查阶段），符合性矩阵已显示 | §5.4 符合性审查 | live-capture | `docs/design/ui/compliance-matrix/screen.png` |
| 07 | `07-comparison-stage.png` | 比价结果界面（表格已加载） | `http://localhost:5173/#/project/:project-id` | 工作台已加载，导航至阶段 4（比价阶段），比价结果表格已显示 | §5.5 比价结果与导出 | live-capture | `docs/design/ui/comparison-export/screen.png` |
...
| 10 | `10-export-result.png` | 导出后的底稿（Excel 导出成功提示或文件预览） | `http://localhost:5173/#/project/:project-id` | 工作台已加载至阶段 4（比价阶段），点击导出后截取导出成功提示或打开导出文件进行截图 | §10 交付成果与验收价值 | export-capture | `docs/design/ui/comparison-export/screen.png` |

### 阶段导航说明

工作台（`/project/:project-id`）的阶段由 React 状态控制，**不体现在 URL 中**。执行者需通过页面顶部的阶段导航标签切换至目标阶段：

| 截图编号 | 阶段索引 | 阶段标签名（界面显示） |
|----------|----------|------------------------|
| 03 | 0 | 导入 / Import |
| 04 | 1 | 标准化 / Standardize |
| 05 | 2 | 归组 / Grouping |
| 06 | 3 | 符合性审查 / Compliance |
| 07、10 | 4 | 比价 / Comparison |
...
1. 在本文档截图清单表的"来源类型"列标注为 `design-fallback`
...
每张截图采集完成后，执行者须更新第四节截图清单表中对应行的"来源类型"列：
